...
import (
	...
	configpkg "admin_api_gateway/internal/pkg/config"
	"context"
	"encoding/json"
...
func NewProducer(config *configpkg.Config, logger *zap.Logger) *producer {
	return &producer{
		...
		userCreate: &kafka.Writer{
			...
			Topic:                  config.Kafka.Topic.ClientCreateToken,
			Balancer:               &kafka.Hash{},
...
func (p *producer) ProduceUserToCreate(ctx context.Context, key string, value *models.RespClient) error {
