...
func (h *handlerV1) UpdateJob(c *gin.Context) {
	...
	respJob, err := h.serviceManager.JobService().GetAllJobs(ctx, &job.GetAll{
		Page:  0,
		Limit: 0,
		Field: "owner_id",
		Value: cast.ToString(claims["sub"]),
	...
	if !t {
		...
		h.log.Error("failed to update job", l.Error(errors.New("job already exists")))
		return
	}
